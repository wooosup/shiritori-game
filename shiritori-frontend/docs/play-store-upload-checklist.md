...
실행 전 준비 체크:
1. `docs/play-store/android-build-readiness-checklist.md`
2. `android/local.properties` 또는 `ANDROID_HOME` 설정
3. `ANDROID_KEYSTORE_PATH`, `ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_ALIAS`, `ANDROID_KEY_PASSWORD`
...
1. `npm run mobile:check-env` 통과
2. `docs/play-store/android-build-readiness-checklist.md`의 SDK/서명 항목 통과
3. 이전 Internal release가 `Pending publication` 상태로 남아 있지 않음
4. 이번 배포에서 확인할 QA 범위 확정
   - 로그인
   - 게임 시작/종료
   - 계정 탈퇴 성공/실패
   - 오프라인 오류 처리
...
## 3-1) 업로드 후 실기기 스모크(최소 1대)

1. opt-in 링크 설치 후 앱 실행
2. Google 로그인 성공 및 홈 복귀 확인
3. 게임 1판 완료(입력, PASS, 종료)
4. 오프라인 전환 후 홈/랭킹/단어장에서 오류+재시도 동작 확인
5. 옵션에서 정책 링크 진입과 계정 탈퇴 동선 확인
6. 결과를 릴리즈 노트 또는 팀 QA 로그에 기록
...
2. `public/legal/terms-ko.html`
3. `public/legal/account-deletion-ko.html`

검증 체크리스트:
1. `docs/play-store/privacy-policy-checklist.md`
2. `docs/play-store/app-access-for-review.md`
3. `docs/play-store/android-build-readiness-checklist.md`
4. `docs/play-store/google-cloud-play-console-checklist.md`
...
4. 로그인 실패(`code 10`)
   - Google Cloud Console Android OAuth client의 패키지명/SHA-1 재확인
5. 빌드는 성공했는데 로그인 후 앱 복귀 실패
   - Supabase Redirect URL의 `shiritori://auth/callback` 등록 여부 확인
